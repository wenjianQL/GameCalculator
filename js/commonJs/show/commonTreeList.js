...
function generateTreeHtml(data, path) {
    ...
    if (item.hasOwnProperty('equName')) {
        str = item['number'].toFixed(2) + " " + item['material'] + " (" + item['equNumber'].toFixed(2) + "*" + item['equName'] + ")";
    } else {
        str = item['number'].toFixed(2) + " " + item['material'];
    }
...
function show_source_material_list_to_pager() {
    ...
    for (let sourceItem in source_data) {
        ...
        if (source_data[sourceItem] >= 0) {
            ...
            // 多余产物
            out_str += "<li>" + source_data[sourceItem].toFixed(1) + " * " + sourceItem + "</li>";
        }
    }
