    ...
    for (let i = 1; i <= max_depth; i++) {
        for (let material in material_list) {
            if (material_list[material]["深度"] == i) {
                material_str += "<tr>" + "<td>" + i + "</td>" + "<td>" + getSatisFactoryImgAndNameNodeStr(material) + "</td>" + "<td>" + material_list[material]["材料数量"].toFixed(2) + "</td>";
                // 设备 + 设备数量
                if (material_list[material]["设备"] == undefined) {
                    material_str += "<td>" + "</td>";
                    material_str += "<td>" + "</td>";
                } else {
                    material_str += "<td>" + getSatisFactoryImgAndNameNodeStr(material_list[material]["设备"]) + "</td>";
