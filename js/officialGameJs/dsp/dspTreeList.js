...
function generateTreeHtml(data, path) {
    ...
    if (item.hasOwnProperty('equName')) {
        str = item['number'].toFixed(2) + " " + getImgAndNameNodeStr(item['material']) + " (" + item['equNumber'].toFixed(2) + "*" + getImgAndNameNodeStr(item['equName']) + ")";
    } else {
        str = item['number'].toFixed(2) + " " + getImgAndNameNodeStr(item['material']);
    }
